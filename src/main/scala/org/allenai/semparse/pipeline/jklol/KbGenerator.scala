...
class KbGenerator(
  ...
  val validParams = Seq("min np count", "min relation count", "training data")
  JsonHelper.ensureNoExtras(params, name, validParams)

  val trainingDataProcessor = new LogicalFormToTrainingData(params \ "training data", fileUtil)
  ...
  val trainingDataFile = trainingDataProcessor.outputFile
  val tripleFile = trainingDataFile.replace("training_data.tsv", "triples.tsv")

  override def inputs = Set(
    (trainingDataFile, Some(trainingDataProcessor))
  )
