...
import org.allenai.semparse.pipeline.science_data.QuestionInterpreter
...
class ScienceQuestionScorer(
  ...
  val validParams = Seq("questions", "question data name", "model")
  JsonHelper.ensureNoExtras(params, name, validParams)

  val questionProcessor = new QuestionInterpreter(params \ "questions", fileUtil)
  val trainer = new Trainer(params \ "model", fileUtil)
  val trainingDataProcessor = trainer.processor
  val modelType = trainer.modelType
  val questionDataName = (params \ "question data name").extract[String]
